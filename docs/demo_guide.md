...
Đảm bảo file `backend/.env` đã có Google Gemini API Key để demo tính năng tóm tắt:
```env
ASR_ENGINE=zipformer
ZIPFORMER_MODEL_DIR=models
ZIPFORMER_NUM_THREADS=4
GEMINI_API_KEY=AIzaSy...  # Điền API Key Gemini của bạn ở đây
```
...
2. Sau khi có kết quả từ Gemini (được gọi từ [llm.py](file:///d:/KProject/AIRecorder/backend/app/services/llm.py)), giao diện hiển thị 3 Tab: Tổng quan, Ý chính, Đầu việc cần làm.
