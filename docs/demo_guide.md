...
### B. File cấu hình `.env` của Backend (Hoặc Cài đặt trực tiếp trên UI)
Đảm bảo file `backend/.env` đã cấu hình API Key để demo tính năng tóm tắt. Bạn cũng có thể thiết lập trực tiếp thông qua hộp thoại Cài đặt (Settings) trên giao diện:
```env
ASR_ENGINE=zipformer
ZIPFORMER_MODEL_DIR=models
ZIPFORMER_NUM_THREADS=4
# API Key của các nhà cung cấp Generative AI (Hỗ trợ nạp làm mặc định lúc startup)
