...
* **Các Nhà cung cấp AI hỗ trợ (AI Providers):**
  ...
  2. **OpenAI ChatGPT API:** Hỗ trợ các dòng mô hình của OpenAI (ví dụ: `gpt-4o`, `gpt-4o-mini`, `gpt-3.5-turbo`).
  3. **Anthropic Claude API:** Hỗ trợ các dòng mô hình của Anthropic (ví dụ: `claude-3-5-sonnet`, `claude-3-5-haiku`).
