...
## 5. Tóm tắt nội dung bằng AI (Generative AI Summary)
...
> [!NOTE]
> Khác với tính năng Nhận diện giọng nói (STT) và Phân biệt người nói (Diarization) chạy hoàn toàn **offline trên máy người dùng**, tính năng Tóm tắt nội dung AI gọi ra các dịch vụ LLM bên ngoài (Gemini, ChatGPT, Claude). Để sử dụng, bạn chỉ cần điền **API Key của nhà cung cấp AI tương ứng** vào mục Cài đặt (Settings) trong ứng dụng.


* **Mã nguồn tích hợp:** [llm.py](file:///d:/KProject/AIRecorder/backend/app/services/llm.py)
* **Các Nhà cung cấp AI hỗ trợ (AI Providers):**
  1. **Google Gemini API:** Hỗ trợ linh hoạt các dòng mô hình Gemini mới nhất (ví dụ: `gemini-2.5-flash`, `gemini-2.5-pro` hoặc phiên bản cập nhật tương đương).
  2. **OpenAI ChatGPT API:** Hỗ trợ linh hoạt các dòng mô hình OpenAI (ví dụ: `gpt-4o`, `gpt-4o-mini` hoặc phiên bản mới hơn).
  3. **Anthropic Claude API:** Hỗ trợ linh hoạt các dòng mô hình Anthropic (ví dụ: `claude-3-5-sonnet`, `claude-3-5-haiku` hoặc phiên bản kế tiếp).
* **Cơ chế cấu hình linh hoạt:** Người dùng có thể dễ dàng chuyển đổi Nhà cung cấp AI, nhập API Key tương ứng, lựa chọn Tên Mô hình bất kỳ và tùy chỉnh System Prompt thông qua giao diện **Cài đặt (Settings Modal)**.
* **Quy trình:**
  1. Hệ thống gửi văn bản đã khôi phục dấu câu lên API của nhà cung cấp đã chọn cùng Prompt hệ thống (System Prompt) được tối ưu hóa.
  2. Kết quả trả về từ API được phân tích và chuyển đổi thành cấu trúc JSON chuẩn gồm 3 phần:
  ...
  3. Kết quả được lưu vào file `summary.json` và hiển thị trên giao diện lớp phủ `AISummaryPanel` của Frontend.
...
5. **Xử lý lỗi mạng / API Key:**
   * Khi mạng yếu hoặc API Key (Gemini, ChatGPT, Claude) bị lỗi/hết hạn, tác vụ tóm tắt sẽ trả về lỗi, chuyển trạng thái ghi âm sang `error` kèm nguồn lỗi `"summarize"`. Toàn bộ nội dung văn bản (transcript) đã dịch trước đó vẫn được bảo toàn nguyên vẹn trên đĩa, người dùng chỉ cần cấu hình lại mạng/key trong Settings rồi bấm Retry Summarize.
