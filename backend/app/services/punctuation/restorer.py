...
class ImprovedPunctuationRestorer:
    def __init__(
        ...
        device="cpu",
        confidence=0.3,
        model_name="dragonSwing/vibert-capu",
        case_confidence=0.0,
        prefer_int8=False,
        ...
        vocab_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "vocabulary_data"
        )

        img_model_path = os.path.join(base_dir, "models", "vibert-capu")
        int8_exists = os.path.exists(
            os.path.join(img_model_path, "vibert-capu.int8.onnx")
        )
        ...
        print(
            f"Loading GecBERTModel ONNX (confidence={confidence}): {model_to_load}..."
        )
    ...
    def restore(self, text, progress_callback=None, pause_hints=None):
        ...
        try:
            ...
            logging.getLogger("core.punctuation").error(
                f"Error during restoration: {e}", exc_info=True
            )
            return text
            ...
            if len(words) < 8 and comma_count > 1:
                ...
                if len(parts) > 1:
                    ...
                    if second_comma_pos != -1:
                        parts[1] = parts[1][:second_comma_pos] + parts[1][
                            second_comma_pos + 1 :
                        ].replace(",", "")
