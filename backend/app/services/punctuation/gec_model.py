...
from .gec_utils import PAD, UNK, START_TOKEN, get_target_sent_by_edits
...
class GecBERTModel:
    def __init__(
        ...
        for model_path in model_paths:
            ...
            logger.info("[GecBERT] ONNX session loaded OK")

            # Load tokenizer
            if os.path.isdir(model_path) and os.path.exists(
                os.path.join(model_path, "vocab.txt")
            ):
            ...
            self.indexers.append(
                self._get_indexer(tokenizer_path, special_tokens_fix=True)
            )
    ...
    def _get_indexer(self, weights_name, special_tokens_fix):
        tokenizer = AutoTokenizer.from_pretrained(
            ...
            do_lower_case=self.lowercase_tokens,
    ...
    def forward(
        ...
        return self.handle_batch(
            text, progress_callback=progress_callback, pause_hints=pause_hints
        )
    ...
    def split_chunks(self, batch, pause_hints=None):
        ...
        for batch_idx, tokens in enumerate(batch):
            ...
            elif num_token > self.chunk_size and num_token < (
                self.chunk_size * 2 - self.overlap_size
            ):
    ...
    def predict(
        ...
        batch_inputs,
        progress_callback=None,
        pause_hints_batch=None,
        orig_tokens_batch=None,
    ):
    ...
    def preprocess(self, token_batch):
        ...
        for indexer in self.indexers:
            prefixed_batch = [
                [START_TOKEN] + sequence[:max_len] for sequence in token_batch
            ]
            batch = indexer(
                prefixed_batch,
                return_tensors="np",
    ...
    def _convert(
        self, logits_list, detect_list, pause_hints_batch=None, orig_tokens_batch=None
    ):
        ...
        for logits, detect_logits, weight in zip(
            logits_list, detect_list, self.model_weights
        ):
        ...
        if pause_hints_batch is not None:
            ...
            for b_idx, hints in enumerate(pause_hints_batch):
                ...
                for w_idx, gap in enumerate(hints):
                    ...
                    current_label = self.vocab.get_token_from_index(
                        current_label_idx, "labels"
                    )

                    if gap >= 1.0:
                        if current_label == "$KEEP":
                            all_class_probs[b_idx, t_idx, self.noop_index] -= 0.2
                            all_class_probs[b_idx, t_idx, self.append_period_index] += (
                                0.2
                            )
                    elif gap >= 0.2:
                        if current_label == "$KEEP":
                            all_class_probs[b_idx, t_idx, self.append_comma_index] += (
                                0.2
                            )
                    elif gap < 0.1:
            ...
            for b_idx, hints in enumerate(pause_hints_batch):
                ...
                for w_idx, gap in enumerate(hints):
                    ...
                    if t_idx >= all_class_probs.shape[1]:
                        break
                    old_label = self.vocab.get_token_from_index(
                        int(before_idx[b_idx, t_idx]), "labels"
                    )
                    new_label = self.vocab.get_token_from_index(
                        int(after_idx[b_idx, t_idx]), "labels"
                    )
                    word = ""
                    if (
                        orig_tokens_batch
                        and b_idx < len(orig_tokens_batch)
                        and w_idx < len(orig_tokens_batch[b_idx])
                    ):
                        word = orig_tokens_batch[b_idx][w_idx]
                    if old_label != new_label:
                        _safe_print(
                            f'  [Pause CHANGED] "{word}" gap={gap:.2f}s: {old_label} -> {new_label}'
                        )
                    else:
                        _safe_print(
                            f'  [Pause kept]    "{word}" gap={gap:.2f}s: {new_label} (unchanged)'
                        )
    ...
    def handle_batch(
        self, full_batch, merge_punc=True, progress_callback=None, pause_hints=None
    ):
        """
        ...
        """
        if self.split_chunk:
            full_batch, indices, pause_hints_chunks = self.split_chunks(
                full_batch, pause_hints=pause_hints
            )
        else:
            indices = None
        ...
        short_ids = [
            i for i in range(len(full_batch)) if len(full_batch[i]) < self.min_len
        ]
        ...
        if self.split_chunk:
            final_batch = [
                self.merge_chunks(final_batch[start:end]) for (start, end) in indices
            ]
        else:
            final_batch = [" ".join(x) for x in final_batch]
        if merge_punc:
            final_batch = [
                re.sub(r"\s+(%s)" % self.punc_str, r"\1", x) for x in final_batch
            ]
